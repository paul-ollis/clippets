...
class AppRunner:
    ...
    def __init__(                                               # noqa: PLR0913
        ...
        self.actions: deque[str] = deque(actions)
    ...
    async def exec_touch(self, arg):
        """Update the timestamp of (touch) the snippet file."""
        self.app.args.snippet_file.touch(exist_ok=True)

    async def exec_text(self, arg):
        """Enter a sequence of characters given as a string."""
        for c in reversed(arg):
            self.actions.appendleft(c)
    ...
    def on_msg(self, m):
        ...
        if name == 'ScreenGreyedOut' and self.app.args.dummy_editor:
            self.actions.appendleft('pause:1.0')
            self.actions.appendleft('snapshot:')
            self.actions.appendleft('pause:0.2')
...
def run_capture(
    ...
    # - Command line opions for the run.
    dims = [80, 30]
        ...
        for rawline in args.snippet_file.read_text().splitlines():
            ...
            if extracting:
                ...
                if line.startswith('# Dims: '):
                    ...
                    add_options.append(line[9:].strip())

    if '--dummy-editor' in add_options:
        script = HERE / 'dummy_editor.py'
        os.environ['CLIPPETS_EDITOR'] = f'python {script} 1.0'
    options = ['--svg-run', *add_options]
    if view_height:
